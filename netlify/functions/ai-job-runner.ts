...
// --- PROMPT TEMPLATES ---
// Centralized prompts for easy editing and A/B testing
const PROMPTS = {
  AUTO_TAG: (categories: string, subcategories: string) => `
  ...
  `,
  HEADSHOT: (hair: string, makeup: string) => `
  ...
  `,
  OUTFIT_MANNEQUIN: (count: number, details: string) => `
...
exports.handler = async (event: any, context: any) => {
  ...
  try {
    ...
    try {
      ...
      console.error(`[AIJobRunner] Error processing ${job.job_type} ${job_id}:`, err);
    }
...
async function uploadImageToStorage(
  ...
  // Simple magic number check for PNG vs JPEG
  const isPng = base64Data.startsWith('iVBORw0KGgo');
  const mimeType = isPng ? 'image/png' : 'image/jpeg';
  // Ensure we strip the data URI prefix if it exists
  const rawBase64 = base64Data.replace(/^data:image\/\w+;base64,/, '');
  const buffer = Buffer.from(rawBase64, 'base64');
  
  // NOTE: If the storagePath forces .jpg but the content is png, this might be okay 
  // depending on Supabase's strictness, but better to match extensions in a real app.
...
async function callGeminiAPI(
  ...
  if (responseType === 'TEXT') {
    ...
    // Try standard inline_data first, then check for potential camelCase variants
...
async function processAutoTag(input: AIJobInput, supabase: any): Promise<any> {
  ...
  const subList = subcategories.map((s: any) => s.name).join(', ');
  ...
  // Map definitions for O(1) lookup
  const defMap = new Map(attrRes.data?.map((d: any) => [d.key, d.id]));
  ...
  // 2. Process Core Wardrobe Updates
  const updates: any = {};
  ...
  const colorAttr = result.attributes?.find((a: any) => a.key === 'color');
  if (colorAttr?.values?.[0]) updates.color_primary = colorAttr.values[0].value;

  if (result.recognized_category) {
    const matchedCat = categories.find((c: any) => c.name.toLowerCase() === result.recognized_category.toLowerCase());
    if (matchedCat) {
      updates.category_id = matchedCat.id;
      if (result.recognized_subcategory) {
        const matchedSub = subcategories.find((s: any) => 
...
async function processProductShot(input: AIJobInput, supabase: any, userId: string): Promise<any> {
  ...
  const { data: existingImages } = await supabase
    ...
    .order('sort_order', { ascending: false }); // Reverse order for easier updating
...
async function processBodyShotGenerate(input: AIJobInput, supabase: any, userId: string): Promise<any> {
  ...
  const [headB64, bodyB64] = await Promise.all([
    downloadImageFromStorage(supabase, headId as string),
...
async function processOutfitRender(input: AIJobInput, supabase: any, userId: string): Promise<any> {
  ...
  if (!bodyId || !headId) throw new Error('Missing body shot or headshot reference');

  // 2. Fetch Images in Parallel
  const wardrobeItemIds = selected.map(s => s.wardrobe_item_id);
  
  // Get image links for all items at once
  ...
  // Determine best image ID for each item
  const imageIdsToDownload: string[] = [];
  wardrobeItemIds.forEach(itemId => {
    const links = linksByItem.get(itemId);
    if (!links?.length) return;
    
    // Prioritize product_shot, then sort_order
  ...
  // 3. Download All Images Parallel
  ...
  if (useMannequin) {
    // Stage 1: Create Mannequin
    const mannequinPrompt = PROMPTS.OUTFIT_MANNEQUIN(itemImages.length, prompt || "No details");
    const mannequinB64 = await callGeminiAPI(mannequinPrompt, itemImages, preferredModel, 'IMAGE');
    
    // Stage 2: Apply to Body (Use Pro model for best composite)
    const compositePrompt = PROMPTS.OUTFIT_FINAL(prompt || "No details");
    finalImageB64 = await callGeminiAPI(compositePrompt, [bodyB64, mannequinB64, headB64], 'gemini-3-pro-image-preview', 'IMAGE');
  } else {
    // Direct Generation
    const directPrompt = PROMPTS.OUTFIT_FINAL(prompt || "No details");
...
async function processOutfitSuggest(input: AIJobInput, supabase: any) { return { message: 'Not implemented' }; }
async function processReferenceMatch(input: AIJobInput, supabase: any) { return { message: 'Not implemented' }; }