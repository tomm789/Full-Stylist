...
async function callGeminiAPI(prompt, images, model = 'gemini-2.5-flash-image', responseType = 'IMAGE') {
  ...
  if (!response.ok || data.error) {
    console.error('[GeminiAPI] API error:', JSON.stringify(data.error || data, null, 2));
  ...
  if (candidate.finishReason && candidate.finishReason !== 'STOP') {
    ...
    throw new Error(`Generation blocked: ${candidate.finishReason} ${msg}`);
  }
  ...
  if (responseType === 'TEXT') {
    ...
    if (!textPart?.text) throw new Error('No text response from API');
...
async function processAutoTag(input, supabase) {
  ...
  const colorAttr = result.attributes?.find(attr => attr.key === 'color');
    ...
    if (matchedCategory) {
      ...
      if (result.recognized_subcategory) {
        const matchedSubcategory = subcategories.find(
          s => s.name.toLowerCase() === result.recognized_subcategory.toLowerCase() &&
               s.category_id === matchedCategory.id
        );
        updates.subcategory_id = matchedSubcategory ? matchedSubcategory.id : null;
      } else {
...
async function processOutfitRender(input, supabase, userId) {
  ...
  let headId = headshot_image_id || userSettings?.headshot_image_id;
  
  if (!bodyId) throw new Error('User must upload body photo before generating outfits');
  if (!headId) throw new Error('User must generate headshot before generating outfits');
  ...
  if (imageIdsToDownload.length === 0) {
...
async function processOutfitSuggest(input, supabase) {
  return { message: 'outfit_suggest not yet implemented' };
}

async function processReferenceMatch(input, supabase) {
