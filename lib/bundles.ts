...
export interface OutfitBundleWithGroups extends OutfitBundle {
  groups?: Array<BundleGroup & { items?: BundleGroupItem[] }>;
}

/**
 * Get outfit bundles for an outfit
 */
export async function getOutfitBundles(outfitId: string): Promise<{
  data: OutfitBundleWithGroups[];
  error: any;
}> {
  try {
    const { data: bundles, error } = await supabase
      ...
      .select(
        `
        *,
        bundle_groups(*, bundle_group_items(*))
      `
      )
...
/**
 * Create outfit bundle with bundle groups
 */
export async function createOutfitBundle(
  ...
  try {
    ...
    // Create bundle groups and items
